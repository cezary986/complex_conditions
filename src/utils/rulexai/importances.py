...
class ConditionImportance:
    ...
    def _get_basic_conditions_for_attribute(self, conditions: List[ElementaryCondition]):
        ...
        if min_point_first.value > float('-inf'):
            points.append(Point(0, float('-inf'), "Left", False))
        min_point_first = min(points, key = attrgetter('value'))
        ...
        if min_point_second.value < float('inf'):
            condition = ElementaryCondition(attribute = attribute, left = min_point_second.value, right= float('inf'), leftClosed = True, rightClosed= False, column_index=self.column_indexes[attribute])
            basic_conditions.append(condition)
