...
class BaseExplainer:
    ...
    def fit_transform(
        self, X: pd.DataFrame, selector=None, y=None, POS=None, basic_conditions: bool = False) -> pd.DataFrame:
        ...
        helper = ConditionImportance(self.model.rules, X, None, basic_conditions)
        if basic_conditions:
            #rules = helper.split_conditions_in_rules(self.model.rules)
            #conditions = helper._get_conditions_with_rules(rules)
            conditions_with_rules = helper._get_conditions_with_rules(self.model.rules)
            conditions = helper._split_conditions_into_basic(list(conditions_with_rules.keys()))
